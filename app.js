document.addEventListener('DOMContentLoaded', () => {
    ...
    // =========================================
    const translations = {
        en: { projectFolder: 'Project Folder', editSchema: 'Edit Schema', newSchema: 'New Schema', dbViewer: 'DB Viewer', analysis: 'Analysis', qrCode: 'QR Code', schemaName: 'Schema Name', schemaVersion: 'Schema Version', save: 'Save', cancel: 'Cancel', newPoint: 'New Point', deletePoint: 'Delete Point', addArrow: 'Add Arrow', deleteArrow: 'Delete Arrow', addColumn: 'Add Column', deleteColumn: 'Delete Column', columns: 'Columns', name: 'Name', unit: 'Unit', nominal: 'Nominal', min: 'Min', max: 'Max', arrows: 'Arrows', arrowWidth: 'Width', arrowColor: 'Color', arrowHead: 'Head', metaLabels: 'Meta Labels', showQR: 'Show QR Code', showDate: 'Show Date', setView: 'Set View', clearView: 'Clear View', confirmDelete: 'Are you sure you want to delete this schema? This cannot be undone.', deleteSuccess: 'Schema deleted successfully.', deleteError: 'Error deleting schema.', schemaInfoMissing: 'Schema Name and Version are required.', saveSuccess: 'Save successful!', saveError: 'Error saving.', confirmExitEditor: 'You have unsaved changes. Are you sure you want to exit?', confirmDeleteRecord: 'Are you sure you want to delete this record?', actions: 'Actions', exportPNG: 'Export as PNG', map: 'Select Map...', selectMapPrompt: 'Select a map to begin.', comment: 'Comment', tolerance: 'Tolerance' },
        pl: { projectFolder: 'Folder Projektu', editSchema: 'Edytuj Schemat', newSchema: 'Nowy Schemat', dbViewer: 'Baza Danych', analysis: 'Analiza', qrCode: 'Kod QR', schemaName: 'Nazwa Schematu', schemaVersion: 'Wersja Schematu', save: 'Zapisz', cancel: 'Anuluj', newPoint: 'Nowy Punkt', deletePoint: 'Usuń Punkt', addArrow: 'Dodaj Strzałkę', deleteArrow: 'Usuń Strzałkę', addColumn: 'Dodaj Kolumnę', deleteColumn: 'Usuń Kolumnę', columns: 'Kolumny', name: 'Nazwa', unit: 'Jednostka', nominal: 'Nominał', min: 'Min', max: 'Max', arrows: 'Strzałki', arrowWidth: 'Grubość', arrowColor: 'Kolor', arrowHead: 'Grot', metaLabels: 'Etykiety Meta', showQR: 'Pokaż Kod QR', showDate: 'Pokaż Datę', setView: 'Ustaw Widok', clearView: 'Wyczyść Widok', confirmDelete: 'Czy na pewno chcesz usunąć ten schemat? Tej operacji nie można cofnąć.', deleteSuccess: 'Schemat usunięty pomyślnie.', deleteError: 'Błąd podczas usuwania schematu.', schemaInfoMissing: 'Nazwa i Wersja Schematu są wymagane.', saveSuccess: 'Zapisano pomyślnie!', saveError: 'Błąd zapisu.', confirmExitEditor: 'Masz niezapisane zmiany. Czy na pewno chcesz wyjść?', confirmDeleteRecord: 'Czy na pewno chcesz usunąć ten rekord?', actions: 'Akcje', exportPNG: 'Eksportuj jako PNG', map: 'Wybierz Mapę...', selectMapPrompt: 'Wybierz mapę, aby rozpocząć.', comment: 'Komentarz', tolerance: 'Tolerancja' },
        de: { projectFolder: 'Projektordner', editSchema: 'Schema bearbeiten', newSchema: 'Neues Schema', dbViewer: 'DB-Ansicht', analysis: 'Analyse', qrCode: 'QR-Code', schemaName: 'Schemaname', schemaVersion: 'Schemaversion', save: 'Speichern', cancel: 'Abbrechen', newPoint: 'Neuer Punkt', deletePoint: 'Punkt löschen', addArrow: 'Pfeil hinzufügen', deleteArrow: 'Pfeil löschen', addColumn: 'Spalte hinzufügen', deleteColumn: 'Spalte löschen', columns: 'Spalten', name: 'Name', unit: 'Einheit', nominal: 'Nominal', min: 'Min', max: 'Max', arrows: 'Pfeile', arrowWidth: 'Breite', arrowColor: 'Farbe', arrowHead: 'Pfeilspitze', metaLabels: 'Meta-Beschriftungen', showQR: 'QR-Code anzeigen', showDate: 'Datum anzeigen', setView: 'Ansicht festlegen', clearView: 'Ansicht löschen', confirmDelete: 'Möchten Sie dieses Schema wirklich löschen? Dies kann nicht rückgängig gemacht werden.', deleteSuccess: 'Schema erfolgreich gelöscht.', deleteError: 'Fehler beim Löschen des Schemas.', schemaInfoMissing: 'Schemaname und -version sind erforderlich.', saveSuccess: 'Erfolgreich gespeichert!', saveError: 'Fehler beim Speichern.', confirmExitEditor: 'Sie haben ungespeicherte Änderungen. Möchten Sie den Editor wirklich verlassen?', confirmDeleteRecord: 'Möchten Sie diesen Datensatz wirklich löschen?', actions: 'Aktionen', exportPNG: 'Als PNG exportieren', map: 'Karte auswählen...', selectMapPrompt: 'Wählen Sie eine Karte aus, um zu beginnen.', comment: 'Kommentar', tolerance: 'Toleranz' },
    };
    ...
    const selectMP = async (id) => {
        ...
        const mp = currentData?.points.find(p => p.id === id);
    ...
    const applyCanvasZoom = (view, animate = true) => {
        ...
        [dom.backgroundImg, dom.overlaySvg, dom.labelsContainer].forEach(el => el.style.transition = transitionStyle);

        [dom.backgroundImg, dom.overlaySvg, dom.labelsContainer].forEach(el => el.style.transform = transform);

        const svgGroup = dom.overlaySvg.querySelector('#svg-content-group');
        if (svgGroup) {
            svgGroup.querySelectorAll('line').forEach(line => {
                const baseWidth = parseFloat(line.dataset.baseStrokeWidth || 2);
                line.setAttribute('stroke-width', baseWidth / scale);
            });
            svgGroup.querySelectorAll('circle').forEach(handle => {
                handle.setAttribute('r', 8 / scale);
            });
        }
        
        dom.labelsContainer.querySelectorAll('.resizing-handle').forEach(el => {
        ...
        if (!animate) {
            setTimeout(() => {
                [dom.backgroundImg, dom.overlaySvg, dom.labelsContainer].forEach(el => el.style.transition = '');
            }, 50);
        }
    ...
    const resetCanvasView = async (animate = true, clearSelection = true) => {
        ...
        const transitionStyle = animate ? '' : 'none';
        [dom.backgroundImg, dom.overlaySvg, dom.labelsContainer].forEach(el => el.style.transition = transitionStyle);
        [dom.backgroundImg, dom.overlaySvg, dom.labelsContainer].forEach(el => el.style.transform = 'translate(0px, 0px) scale(1)');

        const elementsToScale = dom.labelsContainer.querySelectorAll('.mp-label, .meta-label, .resizing-handle');
        elementsToScale.forEach(el => {
            el.style.transform = el.dataset.originalTransform || 'translate(-50%, -50%)';
        });
    ...
    const onEditorZoomWheel = (e) => {
        ...
        applyCanvasZoom({ scale: newScale, offsetX: newOffsetX, offsetY: newOffsetY }, false);
    };
    ...
    const saveHandleToIndexedDB = async (handle) => {
        ...
        store.put({ id: 'lastProject', handle, name: handle.name, savedAt: Date.now() });
        return new Promise((resolve, reject) => {
            transaction.oncomplete = () => { console.log('✅ Directory handle saved to IndexedDB:', handle.name); db.close(); resolve(); };
            transaction.onerror = () => { console.error('❌ Failed to save handle to IndexedDB:', transaction.error); db.close(); reject(transaction.error); };
        });
    };
    ...
    const getHandleFromIndexedDB = async () => {
        try {
            const db = await openHandlesDB();
            const request = db.transaction(['directoryHandles'], 'readonly').objectStore('directoryHandles').get('lastProject');
            return new Promise((resolve, reject) => {
                request.onsuccess = async () => {
                    db.close();
                    if (!request.result?.handle) { resolve(null); return; }
                    const savedHandle = request.result.handle;
                    try {
                        let permission = await savedHandle.queryPermission({ mode: 'readwrite' });
                        if (permission !== 'granted') permission = await savedHandle.requestPermission({ mode: 'readwrite' });
                        resolve(permission === 'granted' ? savedHandle : null);
            ...
            if (mapHandle) {
                try {
                    ...
                    mapData.points.forEach(mp => {
                        if (!mp.arrows) mp.arrows = [{ x1: mp.x1, y1: mp.y1, x2: mp.x2, y2: mp.y2, style: mp.style }];
    ...
    const loadAndDisplayBackground = (bgFilename) => {
        return new Promise(async (resolve, reject) => {
            if (!bgFilename) {
                dom.backgroundImg.src = '';
                dom.backgroundImg.style.display = 'none';
                console.log('🖼️ Cleared background');
                requestRender(); // Render even with no background
                resolve();
                return;
            }
            ...
            if (!bgHandle) {
                ...
                reject(new Error(`Background not found: ${bgFilename}`));
                return;
            }
            ...
            try {
                ...
                dom.backgroundImg.onerror = () => {
                    ...
                    console.error(`❌ Error loading background image: ${bgFilename}`);
                    reject(new Error(`Failed to load background: ${bgFilename}`));
                };
                dom.backgroundImg.src = url;
            } catch (err) {
                console.error(`❌ Error reading background file ${bgFilename}:`, err);
                reject(err);
            }
        });
    };
...
// =========================================
    ...
    const requestRender = () => {
        ...
        requestAnimationFrame(() => {
            renderCanvas();
            appState.ui.renderScheduled = false;
        });
    };
    ...
    const renderCanvas = (recordData = null) => {
        ...
        const filteredPoints = getVisiblePointsForCurrentView();
        
        console.log(`📊 Rendering ${filteredPoints.length}/${currentData.points.length} MPs for background ${appState.ui.activeBackgroundFile || 'none'}`);
    ...
    const updateSingleLabel = (mpId) => {
        ...
        if (mp.type === 'table') {
            ...
            labelHtml = `<div>${escapeHtml(mp.id)}: ${escapeHtml(value)}${escapeHtml(mp.unit)} ${statusText}</div><div class="point-name">${escapeHtml(mp.name)}</div><div class="tolerance">[${escapeHtml(mp.min)}..${escapeHtml(mp.max)}]</div>`;
        }
    ...
    // =========================================
    const toggleEditor = async (open, isNew = false) => {
    ...
    const renderSchemaInspector = () => {
        ...
        contentDiv.innerHTML = `
            <div id="editor-schema-controls">
                ...
                <div class="input-group"><label data-i18n="schemaVersion">${t('schemaVersion')}</label><input type="text" id="editor-schema-version" class="input-field" value="${editorData.version || ''}" ${isExistingSchema ? 'disabled' : ''}></div>
            </div>
        ...
        contentDiv.querySelector('#check-show-qr').addEventListener('change', (e) => { editorData.meta.showQR = e.target.checked; if(e.target.checked && !editorData.meta.qrLabelPos) editorData.meta.qrLabelPos={x:100, y:50}; requestRender(); });
        contentDiv.querySelector('#check-show-date').addEventListener('change', (e) => { editorData.meta.showDate = e.target.checked; if(e.target.checked && !editorData.meta.dateLabelPos) editorData.meta.dateLabelPos={x:100, y:80}; requestRender(); });
    ...
    const renderMPCard = (mp, container) => {
        ...
        card.innerHTML = `<div class="editor-point-header"><input type="text" class="input-field editor-point-id-input" value="${mp.id}" title="Change Point ID"><div class="editor-point-actions"><button class="editor-icon-btn delete" data-action="delete-mp" title="${t('deletePoint')}">🗑️</button></div></div>
            <div class="input-group"><label>${t('name')}</label><input type="text" class="input-field" data-prop="name" value="${mp.name || ''}"></div>
            ${backgroundSelectHtml}
            ${viewControlsHtml}
            <div class="input-group"><label>Type</label><select class="select-field" data-prop="type"><option value="single" ${mp.type==='single'?'selected':''}>Single Value</option><option value="table" ${mp.type==='table'?'selected':''}>Table</option></select></div>
            ${typeHtml}
            ${arrowsHtml}`;
        ...
        card.querySelector('.editor-point-id-input').addEventListener('change', (e) => {
            ...
            if (newId && newId !== mp.id && !appState.ui.editorState.points.some(p => p.id === newId)) {
                mp.id = newId; appState.ui.selectedMPId = newId; appState.ui.editorIsDirty = true; renderSchemaInspector(); requestRender();
        ...
        card.addEventListener('click', async (e) => {
            ...
            if (act === 'delete-mp') { if (confirm(`Delete ${mp.id}?`)) deleteMP(mp.id); }
                ...
                const { scale, offsetX, offsetY } = appState.ui.canvasZoom;
                const rect = dom.canvasWrapper.getBoundingClientRect();
                const centerViewBoxX = ((rect.width / 2) - offsetX) / scale;
                const centerViewBoxY = ((rect.height / 2) - offsetY) / scale;
                const clampedX = Math.max(50, Math.min(950, centerViewBoxX));
                const clampedY = Math.max(50, Math.min(650, centerViewBoxY));
                
                mp.arrows.push({ x1: clampedX - 50, y1: clampedY, x2: clampedX + 50, y2: clampedY, style: {color: '#ff9500', width: 2, head: 'arrow'} }); 
                
                appState.ui.editorIsDirty = true; 
                renderSchemaInspector(); 
                requestRender();
            }
            else if (act === 'delete-arrow') { const idx = parseInt(e.target.closest('.editor-sub-item')?.dataset.arrowIndex || '0', 10); if (!isNaN(idx)) mp.arrows.splice(idx, 1); appState.ui.editorIsDirty = true; renderSchemaInspector(); requestRender(); }
            else if (act === 'add-column') { mp.columns = mp.columns || []; mp.columns.push({name:'New Col', unit:'mm', nominal:0, min:-0.1, max:0.1}); appState.ui.editorIsDirty = true; renderSchemaInspector(); requestRender(); }
            else if (act === 'delete-column') { const idx = parseInt(e.target.closest('.editor-sub-item')?.dataset.colIndex || '0', 10); if (!isNaN(idx)) mp.columns.splice(idx, 1); appState.ui.editorIsDirty = true; renderSchemaInspector(); requestRender(); }
        ...
        card.querySelectorAll('[data-prop]').forEach(input => {
            const eventType = input.matches('select, input[type="color"]') ? 'change' : 'input';
            input.addEventListener(eventType, async (e) => {
                appState.ui.editorIsDirty = true;
                const path = e.target.dataset.prop.split('.');
                let val = e.target.type === 'checkbox' ? e.target.checked : e.target.value;
                if (path[0] === 'backgroundId') val = val === '' ? null : val;
                if (e.target.type === 'number' || path[path.length - 1].match(/nominal|min|max|width/)) val = parseFloat(val.replace(',', '.')) || val;
                ...
                if (path[0] === 'type') renderSchemaInspector(); 
                requestRender();
            });
        });
    };
    ...
    const onEditorMouseDown = (e) => {
        ...
        if (!target) target = (e.target.tagName === 'circle' || e.target.classList.contains('mp-label') || e.target.classList.contains('meta-label')) ? e.target : null;

        if (target) {
            e.preventDefault();
            e.stopPropagation();
            
            const mpId = target.dataset?.mpId || target.closest('.mp-label')?.dataset.mpId;
            if (mpId) {
                selectMP(mpId);
                ...
                let dragType = target.closest('.mp-label') ? 'label' : handleType;
                
                const foundMp = appState.ui.editorState.points.find(p => p.id === mpId);
                if (foundMp) appState.ui.dragging = { type: dragType || 'label', mp: foundMp, arrowIndex };
    ...
    const addMP = async () => {
        ...
        const selectedMP = appState.ui.selectedMPId ? points.find(p => p.id === appState.ui.selectedMPId) : null;
        ...
        try {
            ...
            for (const [fileName, handle] of Object.entries(appState.fileHandles.maps)) {
                try {
                    ...
                    maps.push({ fileName, name: fileName.replace('.map.json', ''), meta: mapData.meta || {} });
                } catch (err) { console.warn(`Could not read map ${fileName}:`, err); }
            }
                ...
                records: appState.data.db.map(record => {
                    ...
                    Object.keys(record).forEach(key => {
                        if (key.endsWith('_Value') && record[key] && record[key].toString().trim() !== '') {
                            ...
                            const value = record[`${mpId}_Value`], min = record[`${mpId}_Min`], max = record[`${mpId}_Max`];
                    ...
                    return {
                        ...
                        schemaName: record.SchemaName || '', schemaVersion: record.SchemaVersion || '',
                        inspector: 'Inspector', overallStatus: record.OverallStatus || 'OK',
                        comment: record.Comment || '', measurements: measurements
                    };
                })
            };
    ...
    const saveAndExport = async () => {
        ...
        if (!qr) { alert("QR Code required."); return; }
        ...
        try {
            ...
            const newRecord = {
                ...
                Timestamp: new Date().toISOString(), QRCode: qr, SchemaName: sn, SchemaVersion: sv||'1',
                OverallStatus: overallStatus, Comment: document.getElementById('measurement-comment').value.trim(), ...mpData
            };
            const allHeaders = new Set(appState.data.dbHeaders);
            Object.keys(newRecord).forEach(k => allHeaders.add(k));
            appState.data.dbHeaders = [...allHeaders];
            appState.data.db.push(newRecord);
            ...
            await generateAllVisualizationImages(qr, appState.data.currentMap.fileName, newRecord);
    ...
    const exportPNG = async ({ 
        fromSave = false, recordData = null, mapData = null, 
        saveToFile = false, showAlertOnSuccess = true, backgroundId = null, 
        visibleMPs = null, fileName = null, view = null 
    } = {}) => {
        return new Promise(async (resolve, reject) => {
            const cMap = mapData || appState.data.currentMap;
            
            let backgroundFileName;
            const bg = backgroundId ? cMap.meta.backgrounds.find(b => b.id === backgroundId) : null;
            ...
            if (!backgroundFileName) return reject("Schema has no background configured");
            
            const bgH = appState.fileHandles.backgrounds[backgroundFileName];
            if (!bgH) return reject(`Background file not found: ${backgroundFileName}`);
            ...
            img.onload = () => {
                ...
                const effectiveView = view || { scale: 1, offsetX: 0, offsetY: 0 };
                
                // Always use a fixed output size for consistency
                cv = document.createElement('canvas');
                cv.width = 1200;
                cv.height = 800;
                ctx = cv.getContext('2d');
                
                const cropW = nw / effectiveView.scale;
                const cropH = nh / effectiveView.scale;
                
                // The new transform system uses top-left offsets, making this simpler
                const cropX = -effectiveView.offsetX * (nw / dom.canvasWrapper.offsetWidth);
                const cropY = -effectiveView.offsetY * (nh / dom.canvasWrapper.offsetHeight);
                
                // Draw cropped background
                ctx.drawImage(img, cropX, cropY, cropW, cropH, 0, 0, cv.width, cv.height);
                
                // Adjust scale and offset for rendering MPs in cropped view
                const viewScale = cv.width / cropW;
                scale = (Math.min(nw / VIEWBOX_WIDTH, nh / VIEWBOX_HEIGHT)) * viewScale;
                ox = -cropX * viewScale;
                oy = -cropY * viewScale;
                
                URL.revokeObjectURL(bgUrl);
                const cRec = recordData;
                const getVal = (mpId, colName) => {
                    const selector = colName 
                        ? `.mp-row[data-mp-id="${mpId}"] input[data-col-index="${mp.columns.findIndex(c => c.name === colName)}"]`
                        : `.mp-row[data-mp-id="${mpId}"] input`;
                    return cRec ? (colName ? cRec[`${mpId}_${colName}_Value`] : cRec[`${mpId}_Value`]) : document.querySelector(selector)?.value || '';
                };
                ...
                pointsToRender.forEach(mp => {
                    (mp.arrows||[{x1:mp.x1,y1:mp.y1,x2:mp.x2,y2:mp.y2,style:mp.style}]).forEach(a => {
                        ...
                        ctx.beginPath(); ctx.moveTo(x1,y1); ctx.lineTo(x2,y2); ctx.strokeStyle=col; ctx.lineWidth=lw; ctx.lineCap='round'; ctx.stroke();
                        const head = (fx,fy,tx,ty) => {
                            const hl=10*lw/2, ang=Math.atan2(ty-fy,tx-fx);
                            ctx.beginPath(); ctx.moveTo(tx,ty); ctx.lineTo(tx-hl*Math.cos(ang-Math.PI/6),ty-hl*Math.sin(ang-Math.PI/6)); ctx.lineTo(tx-hl*Math.cos(ang+Math.PI/6),ty-hl*Math.sin(ang+Math.PI/6)); ctx.closePath(); ctx.fillStyle=col; ctx.fill();
                        };
                        if(a.style?.head==='arrow'||mp.style?.head==='arrow') head(x1,y1,x2,y2);
                        else if(a.style?.head==='double'||mp.style?.head==='double') { head(x1,y1,x2,y2); head(x2,y2,x1,y1); }
                    });
                    ...
                    if(mp.type==='table') {
                        ...
                        (mp.columns||[]).forEach((c, i) => {
                            ...
                            if(v && v.trim()!=='') { any=true; const n=parseFloat(v.replace(',','.')); if(isNaN(n)||n<c.min||n>c.max) allOk=false; }
                        });
                        if(any) { st=allOk?'OK':'NOK'; sCol=allOk?'#34c759':'#ff3b30'; } else st='';
                        valTxt = mp.id;
                    } else {
                        const v=getVal(mp.id);
                        if(v!=='') { const n=parseFloat(v.replace(',','.')); if(!isNaN(n)&&n>=mp.min&&n<=mp.max) { st='OK'; sCol='#34c759'; } else { st='NOK'; sCol='#ff3b30'; } } else st='';
                        valTxt = `${mp.id}: ${v}${mp.unit}`;
                    }
                    if(st || valTxt) {
                        const lx=(mp.labelX||(mp.arrows?.[0]?.x2||mp.x2))*scale+ox, ly=(mp.labelY||(mp.arrows?.[0]?.y2||mp.y2))*scale+oy;
                        ctx.font=`600 ${16*scale}px sans-serif`; ctx.textAlign='center'; ctx.textBaseline='middle';
                        const statusText = st ? ` ${st}` : ''; const valueText = `${valTxt}${statusText}`;
                        const m1=ctx.measureText(valueText); ctx.font=`400 ${14*scale}px sans-serif`; const m2=ctx.measureText(mp.name);
                        const tolText = mp.type === 'table' ? '' : `[${mp.min}..${mp.max}]`; ctx.font=`400 ${12*scale}px sans-serif`; const m3=ctx.measureText(tolText);
                        const w=Math.max(m1.width,m2.width,m3.width)+24*scale; const h=(1.4*16*scale)*3.5;
                        ctx.fillStyle='white'; ctx.strokeStyle=sCol; ctx.lineWidth=2*scale; ctx.beginPath(); ctx.roundRect(lx-w/2,ly-h/2,w,h,[8*scale]); ctx.fill(); ctx.stroke();
                        ctx.fillStyle='#0B0F10'; ctx.font=`600 ${16*scale}px sans-serif`; ctx.fillText(valueText, lx, ly-h*0.25);
                        ctx.font=`400 ${14*scale}px sans-serif`; ctx.fillStyle='#6c757d'; ctx.fillText(mp.name, lx, ly+h*0.05);
                        if (tolText) { ctx.font=`400 ${12*scale}px sans-serif`; ctx.fillStyle='#6c757d'; ctx.fillText(tolText, lx, ly+h*0.3); }
                    }
                });
                ...
                const drawMeta=(txt,p)=>{
                    if(!p)return; const x=p.x*scale+ox, y=p.y*scale+oy; ctx.font=`600 ${16*scale}px sans-serif`;
                    const m=ctx.measureText(txt), w=m.width+16*scale, h=32*scale;
                    ctx.fillStyle='white'; ctx.strokeStyle='#6c757d'; ctx.lineWidth=2*scale; ctx.beginPath(); ctx.roundRect(x-w/2,y-h/2,w,h,[8*scale]); ctx.fill(); ctx.stroke();
                    ctx.fillStyle='#0B0F10'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText(txt,x,y);
                };
                ...
                cv.toBlob(async (b) => {
                    ...
                    if(saveToFile) {
                        ...
                        const a=document.createElement('a'); a.href=URL.createObjectURL(b); a.download=fn; a.click(); URL.revokeObjectURL(a.href);
                        if(showAlertOnSuccess) alert(t('exportSuccess'));
                    }
                    resolve();
                }, 'image/png');
    ...
    const generateAllVisualizationImages = async (qrCode, schemaName, recordData) => {
        ...
        try {
            ...
            for (const bgId of uniqueBackgrounds) {
                ...
                await exportPNG({
                    recordData, mapData, saveToFile: true, showAlertOnSuccess: false,
                    backgroundId: bgId, visibleMPs: mpsForThisBg,
            ...
            for (const mp of mapData.points) {
                ...
                await exportPNG({
                    recordData, mapData, saveToFile: true, showAlertOnSuccess: false,
                    backgroundId: bgId, visibleMPs: [mp],
                    fileName: `${qrCode}_${mp.id}.png`, view: mp.view
                });
            }
    ...
    const showSuggestions = (items, cell) => {
        ...
        appState.ui.formulaSuggestions = { ...appState.ui.formulaSuggestions, items, activeIndex: -1, targetCell: cell };
        ...
        try {
            ...
            const modalBody = dom.dbTable.closest('.modal-body');
    ...
    const hideSuggestions = () => {
        ...
        setTimeout(() => {
            if (!appState.ui.formulaSuggestions.visible) popup.style.display = 'none';
        }, 200);
    };
    ...
    const handleSuggestionNavigation = (e) => {
        ...
        if (e.key === 'ArrowDown') newIndex = (activeIndex + 1) % items.length;
        else if (e.key === 'ArrowUp') newIndex = (activeIndex - 1 + items.length) % items.length;
        else if (e.key === 'Tab' || e.key === 'Enter') {
            e.preventDefault();
            if (activeIndex > -1) insertSuggestion();
            else if (items.length === 1) { appState.ui.formulaSuggestions.activeIndex = 0; insertSuggestion(); }
            return true;
        } else if (e.key === 'Escape') { e.preventDefault(); hideSuggestions(); return true; }
    ...
    const insertSuggestion = () => {
        ...
        if (activeIndex < 0 || !targetCell || !items?.[activeIndex]) return;
        ...
        try {
            ...
            let textToInsert, textToReplace, cursorOffset;
            ...
            try {
                ...
                if (targetCell.firstChild) {
                    const safePos = Math.max(0, Math.min(newCaretPos, targetCell.firstChild.length));
                    range.setStart(targetCell.firstChild, safePos);
                    range.collapse(true);
                    sel.removeAllRanges();
                    sel.addRange(range);
                }
            } catch (e) { targetCell.focus(); }

            setTimeout(() => updateFormulaSuggestions(targetCell), 50);
        } catch (err) { console.error('Error inserting suggestion:', err); }
    };
    ...
    const renderDbViewer = () => {
        ...
        const thead = tableEl.createTHead(), headerRow = thead.insertRow();
        if (appState.data.dbHeaders) {
            appState.data.dbHeaders.forEach((headerText, index) => {
                ...
                if (!isReadOnly) {
                    ...
                    th.addEventListener('dragover', (e) => { e.preventDefault(); e.currentTarget.classList.add('drag-over'); });
                    th.addEventListener('dragleave', (e) => e.currentTarget.classList.remove('drag-over'));
                    th.addEventListener('drop', handleDbColumnDrop);
                    th.addEventListener('contextmenu', (e) => showDbHeaderContextMenu(e, headerText));
                }
                if (appState.data.columnFormulas[headerText]) th.innerHTML += ' <span class="fx-icon">fx</span>';
                headerRow.appendChild(th);
            });
        }

        const actionsTh = document.createElement('th'); actionsTh.textContent = t('actions'); headerRow.appendChild(actionsTh);
        const addColTh = document.createElement('th'); addColTh.style.textAlign = 'center';
        const addBtn = document.createElement('button'); addBtn.textContent = '+'; addBtn.className = 'btn btn-secondary'; addBtn.style.height = '24px'; addBtn.style.padding = '0 8px'; addBtn.addEventListener('click', addNewDbColumn); addColTh.appendChild(addBtn);
        ...
        filteredData.forEach((r, rowIndex) => {
            const row = tbody.insertRow(); row.dataset.recordId = r.RecordId;
            if (appState.data.dbHeaders) {
                appState.data.dbHeaders.forEach((k, colIndex) => {
                    const c = row.insertCell(); c.textContent = r[k] || '';
                    if (c.textContent === '#ERROR!') c.classList.add('formula-error');

                    if(!READONLY_COLS.includes(k)) {
                        c.contentEditable=true; c.dataset.header=k;
                        c.addEventListener('focus', () => updateFormulaSuggestions(c));
                        c.addEventListener('input', () => updateFormulaSuggestions(c));
                        c.addEventListener('keydown', (e) => {
                            ...
                            if (e.key === 'Enter' && !e.shiftKey) {
                                ...
                                hideSuggestions();
                                if (formula?.startsWith('=')) applyFormulaToColumn(k, formula);
                                else row.parentElement.rows[rowIndex + 1]?.cells[colIndex]?.focus();
                            }
                        });
                        c.addEventListener('blur', () => setTimeout(hideSuggestions, 200));
                    } else c.classList.add('readonly-col');
                    if(k==='QRCode') c.dataset.qrCodeForSearch=(r[k]||'').toLowerCase();
                });
            }
            row.insertCell().innerHTML = `<button class="db-export-btn" title="${t('exportPNG')}">📷</button><button class="db-delete-btn" title="${t('deleteSchema')}">🗑️</button>`;
        });
    };
    ...
    const handleDbColumnDrop = (e) => {
        ...
        const toTh = e.target.closest('th');
        if (!toTh) return;
    ...
    const showDbHeaderContextMenu = (e, headerText) => {
        e.preventDefault(); hideSuggestions(); const menu = dom.dbHeaderContextMenu;
        menu.style.display = 'block'; menu.style.left = `${e.pageX}px`; menu.style.top = `${e.pageY}px`;
        appState.ui.dbHeaderContextMenu = { visible: true, target: headerText, x: e.pageX, y: e.pageY };
        menu.querySelector('[data-action="delete"]').classList.toggle('disabled', READONLY_COLS.includes(headerText));
        menu.querySelector('[data-action="rename"]').classList.toggle('disabled', READONLY_COLS.includes(headerText));
    };

    const hideDbHeaderContextMenu = () => { dom.dbHeaderContextMenu.style.display = 'none'; appState.ui.dbHeaderContextMenu.visible = false; };
    ...
    const addNewDbColumn = () => {
        ...
        if (colName && !appState.data.dbHeaders.includes(colName)) {
            ...
            renderDbViewer();
        } else if (colName) alert("Column name already exists.");
    };
    ...
    const renameDbColumn = (oldName, newName) => {
        ...
        if (index > -1 && newName && !appState.data.dbHeaders.includes(newName)) {
            ...
            appState.data.db.forEach(row => { row[newName] = row[oldName]; delete row[oldName]; });
            if (appState.data.columnFormulas[oldName]) { appState.data.columnFormulas[newName] = appState.data.columnFormulas[oldName]; delete appState.data.columnFormulas[oldName]; }
            renderDbViewer();
        } else if (newName) alert("New column name is invalid or already exists.");
    };
    ...
    const applyFormulaToColumn = (colName, formula) => {
        if (formula.trim() === '=' || formula.trim() === '') delete appState.data.columnFormulas[colName];
        else appState.data.columnFormulas[colName] = formula;
        appState.data.db.forEach(row => row[colName] = evaluateFormula(formula, row));
        renderDbViewer();
    };

    const saveDbChanges = async () => {
        try {
            dom.dbTable.querySelectorAll('tbody tr').forEach(tr => {
                const r = appState.data.db.find(rec => rec.RecordId === tr.dataset.recordId);
                if (!r) return;
                tr.querySelectorAll('td[contenteditable]').forEach(c => r[c.dataset.header] = c.textContent);
            });
            ...
            await writeFile(appState.fileHandles.db, serializeCSV(appState.data.dbHeaders, appState.data.db));
            alert(t('saveSuccess'));
            renderDbViewer(); // Re-render to show persistent computed values
        } catch(e) { console.error(e); alert(t('saveError')); }
    };

    const openAnalysis = () => {
        if (appState.data.db.length === 0) { alert("Database empty."); return; }
    ...
    const updateAnalysisUI = () => {
        ...
        if (appState.ui.analysisMode === 'trend') {
            ...
            filterAnalysisData();
            populateAnalysisMPs();
        } else updateProfileRecordSelector();
        generateChart();
    };
    ...
    const filterAnalysisData = () => {
        const df = dom.analysisDateFrom.valueAsDate, dt = dom.analysisDateTo.valueAsDate, qr = dom.analysisQrSearch.value.toLowerCase(), schemas = [...dom.analysisSchemasList.querySelectorAll('input:checked')].map(i => i.value);
    ...
    const populateAnalysisMPs = () => {
        ...
        dom.analysisMpsList.innerHTML = [...h].filter(k => k.endsWith('_Value')).sort().map(k => {
            ...
            return `<div class="mp-selection-item"><label><input type="checkbox" name="mp" value="${mp}" checked data-mp="${mp}"> ${mp}</label><input type="checkbox" class="mp-tolerance-toggle" data-mp="${mp}" title="Show tolerance lines"></div>`;
        }).join('');
    ...
    const updateProfileRecordSelector = () => {
        ...
        if (recs.length > 0 && dom.analysisRecordSelect.selectedOptions.length === 0) dom.analysisRecordSelect.selectedIndex = 0;
    ...
    const generateChart = () => {
        ...
        if (mode === 'trend') {
            ...
            mps.forEach((mp,i) => {
                ...
                datasets.push({ label: mp, data: data.map(r => parseFloat(r[`${mp}_Value`]?.replace(',','.'))||null), borderColor: col, backgroundColor: col+'80', tension: 0.1, spanGaps:true });
                if(tols.includes(mp) && data[0]) {
                     datasets.push({ label: `${mp} Nom`, data: Array(data.length).fill(parseFloat(data[0][`${mp}_Nominal`]?.replace(',','.'))), borderColor: 'green', borderDash:[5,5], pointRadius:0, fill:false });
                     datasets.push({ label: `${mp} Min`, data: Array(data.length).fill(parseFloat(data[0][`${mp}_Min`]?.replace(',','.'))), borderColor: 'orange', borderDash:[5,5], pointRadius:0, fill:false });
                     datasets.push({ label: `${mp} Max`, data: Array(data.length).fill(parseFloat(data[0][`${mp}_Max`]?.replace(',','.'))), borderColor: 'red', borderDash:[5,5], pointRadius:0, fill:false });
                }
            });
        } else {
             ...
             recs.forEach((r,i) => {
                 ...
                 datasets.push({ label: r.QRCode, data: labels.map(l => parseFloat(r[`${l}_Value`]?.replace(',','.'))||null), borderColor: col, backgroundColor: col+'80', spanGaps:true });
             });
             if(recs[0]) {
                 datasets.push({ label: t('nominal'), data: labels.map(l => parseFloat(recs[0][`${l}_Nominal`]?.replace(',','.'))), borderColor: 'green', borderDash:[5,5], pointRadius:0 });
                 datasets.push({ label: t('min'), data: labels.map(l => parseFloat(recs[0][`${l}_Min`]?.replace(',','.'))), borderColor: 'orange', borderDash:[5,5], pointRadius:0 });
                 datasets.push({ label: t('max'), data: labels.map(l => parseFloat(recs[0][`${l}_Max`]?.replace(',','.'))), borderColor: 'red', borderDash:[5.5], pointRadius:0 });
             }
        }
        appState.ui.chartInstance = new Chart(ctx, {
            ...
            options: {
                ...
                scales: { x: { title: { display: !!dom.analysisAxisX.value, text: dom.analysisAxisX.value } }, y: { title: { display: !!dom.analysisAxisY.value, text: dom.analysisAxisY.value } } },
                plugins: {
                    zoom: { zoom: { wheel: { enabled: true }, mode: 'x', onZoomComplete: () => dom.chartResetViewBtn.classList.remove('hidden') }, pan: { enabled: true, mode: 'x', onPanComplete: () => dom.chartResetViewBtn.classList.remove('hidden') } },
                    tooltip: { callbacks: { label: (c) => `${c.dataset.label||''}: ${c.parsed.y!==null?formatNumber(c.parsed.y):''}` } }
                },
                onClick: (e, el, chart) => {
    ...
    dom.btnProjectFolder.addEventListener('click', async () => {
        try {
            ...
            if (error.name !== 'AbortError') console.error('❌ Folder selection failed:', error);
        }
    });
    ...
    dom.backgroundUploader.addEventListener('change', async (e) => {
        ...
        if (files.length === 0 || !appState.ui.editorState) return;

        appState.ui.editorState.meta.backgrounds = appState.ui.editorState.meta.backgrounds || [];
        ...
        for (const f of files) {
            try {
                ...
                const fn = `${bgId}.${f.name.split('.').pop()}`;
                
                const fileHandle = await getOrCreateFile(await getOrCreateDir(appState.projectRootHandle, 'backgrounds'), fn);
                await writeFile(fileHandle, f);
                
                appState.fileHandles.backgrounds[fn] = fileHandle;
                appState.ui.editorState.meta.backgrounds.push({ id: bgId, name: f.name.replace(/\.[^/.]+$/, ''), fileName: fn });
    ...
    dom.btnCloseEditor.addEventListener('click', () => { if (!appState.ui.editorIsDirty || confirm(t('confirmExitEditor'))) toggleEditor(false); });
    ...
    dom.qrCodeInput.addEventListener('input', () => requestRender());
    ...
    dom.dbSearchInput.addEventListener('input', (e) => {
        ...
        dom.dbTable.querySelectorAll('tbody tr').forEach(r => r.style.display = r.querySelector('[data-qr-code-for-search]')?.dataset.qrCodeForSearch.includes(t) ? '' : 'none');
    });
    dom.dbTable.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('.db-delete-btn')) {
            ...
            const mapHandle = r ? appState.fileHandles.maps[`${r.SchemaName}_v${r.SchemaVersion}.map.json`] : null;
            if (r && mapHandle) {
                try {
                    const md = JSON.parse(await readFile(mapHandle));
    ...
    document.addEventListener('click', (e) => { if (appState.ui.dbHeaderContextMenu.visible && !dom.dbHeaderContextMenu.contains(e.target)) hideDbHeaderContextMenu(); });
    ...
    [dom.analysisDateFrom, dom.analysisDateTo, dom.analysisQrSearch].forEach(el => el.addEventListener('input', () => { filterAnalysisData(); populateAnalysisMPs(); generateChart(); }));
    dom.analysisSchemasList.addEventListener('change', () => { filterAnalysisData(); populateAnalysisMPs(); generateChart(); });
    dom.btnExportPNG.addEventListener('click', () => { if(appState.ui.chartInstance) { const a=document.createElement('a'); a.href=appState.ui.chartInstance.toBase64Image(); a.download='chart.png'; a.click(); } });
    dom.btnExportCSV.addEventListener('click', () => {
        ...
        if(d.length>0) { const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([serializeCSV(Object.keys(d[0]), d)], {type:'text/csv'})); a.download='analysis_export.csv'; a.click(); } else alert("No data");
    });
